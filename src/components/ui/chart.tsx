...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // SECURE: Generate CSS variables without dangerous HTML injection
  const generateCSSVariables = () => {
    ...
    Object.entries(THEMES).forEach(([theme, prefix]) => {
      ...
      colorConfig.forEach(([key, itemConfig]) => {
        ...
        if (color) {
          // SECURE: Validate color value to prevent CSS injection
      ...
      if (themeRules.length > 0) {
        cssRules.push(`${prefix} [data-chart="${id}"] { ${themeRules.join(' ')} }`)
      }
    })
  ...
  // SECURE: Use CSS-in-JS instead of dangerouslySetInnerHTML
...
// SECURE: Validate CSS color values to prevent injection
function isValidCSSColor(color: string): boolean {
  ...
  return safeColorPatterns.some(pattern => pattern.test(color))
}
